...
- ✅ **Мультивалютність** - підтримка UAH, USD, EUR з автоматичною конвертацією
...
- ✅ **Доходи та витрати** - облік фінансових операцій з категоріями
- ✅ **Звіти за періодами** - сьогодні, тиждень, місяць, рік
- ✅ **HTML звіти** - інтерактивні звіти з графіками Chart.js
- ✅ **Детальна аналітика** - порівняння з попереднім періодом, динаміка по днях
- ✅ **Загальні фінанси** - перегляд балансу (доходи - витрати) за період з візуальними індикаторами
- ✅ **Зміна мови** - перемикання між мовами в налаштуваннях
- ✅ **Категорії** - управління власними категоріями доходів та витрат
...
- **Jinja2** - шаблонізація HTML звітів
- **Chart.js** - інтерактивні графіки та діаграми
- **NBU API** - курси валют Національного банку України
- **ExchangeRate-API** - резервний сервіс курсів валют
...
│   ├── category_repository.py # Репозиторій для категорій
│   ├── report_repository.py  # Репозиторій для звітів
│   ├── currency_converter.py # Конвертер валют (NBU API)
...
│   └── report.py             # Моделі звітів (ReportData, PeriodComparison)
│
├── templates/                # Шаблони HTML
│   ├── report.html           # Шаблон звіту
│   └── styles.css            # Стилі для звітів
│
├── reports/                  # Згенеровані HTML звіти
│   └── *.html                # Автоматично генеруються
│
├── helper_scripts/           # Допоміжні скрипти
│   ├── fill_test_data.py     # Заповнення тестовими даними
...
- `user_repository.py` - CRUD операції для користувачів
- `income_repository.py` - CRUD операції для доходів (з мультивалютністю)
- `expense_repository.py` - CRUD операції для витрат (з мультивалютністю)
- `category_repository.py` - CRUD операції для категорій
- `report_repository.py` - Генерація звітів з розбивкою по валютах
- `currency_converter.py` - Конвертація валют через NBU API
...
### Джерела курсів валют

1. **NBU API** (основне джерело)
   - Офіційні курси Національного банку України
   - Оновлюються щодня
   - https://bank.gov.ua/NBUStatService/v1/statdirectory/exchange

2. **ExchangeRate-API** (резервне джерело)
   - Використовується якщо NBU недоступний
   - https://api.exchangerate-api.com/v4/latest/UAH

### Особливості

- **Збереження оригіналу**: Всі транзакції зберігаються в оригінальній валюті
- **Автоматична конвертація**: Суми конвертуються в дефолтну валюту користувача для підсумків
- **Кешування курсів**: Курси кешуються на 1 годину для швидкодії
- **Інформаційна конвертація**: Показ конвертації з символом ≈ (приблизно)
- **Розбивка по валютах**: Детальні звіти показують суми в кожній валюті окремо
...
*Детальний режим:*
- Розбивка по валютах з конвертацією
- Tooltips з повною інформацією
- При disabled - показує спрощений вигляд

### Як це працює

1. **Додавання транзакції**: Користувач може вибрати валюту (UAH/USD/EUR)
2. **Збереження**: Сума зберігається в оригінальній валюті
3. **Конвертація**: При показі звітів суми конвертуються через актуальні курси
4. **Відображення**: Показується оригінал + конвертація з символом ≈

### Приклад використання

```python
# Користувач додає дохід: 100 USD
# Зберігається: amount=100, currency='USD'

# При перегляді звіту (курс 42 UAH/USD):
# Показується: "100.00 $ (≈ 4,200.00 ₴)"

# Загальна сума доходів:
# "≈ 4,200.00 ₴" (конвертовано в дефолтну валюту)
```
...
- ✅ Додавання доходів з власними категоріями
- ✅ Додавання витрат з власними категоріями
- ✅ Управління категоріями (додавання, редагування, видалення)
- ✅ Перегляд доходів за період (сьогодні, тиждень, місяць, рік)
- ✅ Перегляд витрат за період (сьогодні, тиждень, місяць, рік)
- ✅ Детальні текстові звіти з аналітикою
- ✅ Швидкі компактні звіти
- ✅ **HTML звіти з графіками:**
  - Стовпчикова діаграма доходів vs витрат
  - Динаміка по днях (доходи, витрати, баланс)
  - Кругові діаграми розподілу по категоріях
  - Інтерактивні графіки з Chart.js
  - Адаптивний дизайн для мобільних
  - Пошук по транзакціях
  - Порівняння з попереднім періодом
  - **Мультивалютна підтримка:**
    - Спрощений режим: показує тільки конвертовану суму з ≈
    - Детальний режим: розбивка по валютах + конвертація
    - Tooltips з повною інформацією по всіх валютах
    - Disabled категорії показують спрощений вигляд
    - Реальні курси валют через NBU API
